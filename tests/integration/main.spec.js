...
jest.mock('obsidian', () => {
  class Plugin {
    constructor(app, manifest) {
      ...
      this.__commands = [];     // tests read this
      this._events = [];
    ...
    registerEvent(e) { this._events.push(e); return e; }
  ...
  class Setting {
    ...
    addText(cb) {
      ...
      cb(control);
      // tests call Setting._text[i]('value')
      Setting._text.push((v) => control._onChange && control._onChange(v));
      return this;
    }
    ...
    addToggle(cb) {
      ...
      cb(control);
      Setting._toggle.push((v) => control._onChange && control._onChange(v));
      return this;
    }

    // Textarea control (for Excluded folders)
    addTextArea(cb) {
      const control = {
        _onChange: null,
        setValue() { return this; },
...
describe('main.js integration', () => {
  ...
  describe('Excluded folders in manual command', () => {
    it('skips when active file is in an excluded folder', async () => {
      const PluginCtor = require('../../main.js');
      const app = makeApp();
      const plugin = new PluginCtor(app, { id: 'obsidian-toc-generator' });

      jest.spyOn(plugin, 'loadData').mockResolvedValue({ debounce_ms: 0, excludedFolders: ['Templates'] });

      let registeredCommand;
      jest.spyOn(plugin, 'addCommand').mockImplementation((cmd) => {
        if (cmd.id === 'mdnav-toc-run-on-active') registeredCommand = cmd;
      });

      await plugin.onload();

      const { TFile } = require('obsidian');
      const file = Object.assign(new TFile(), { path: 'Templates/doc.md', extension: 'md' });
      app.workspace.getActiveFile = () => file;
